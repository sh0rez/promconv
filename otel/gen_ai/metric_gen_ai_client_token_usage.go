...
func NewClientTokenUsage() ClientTokenUsage {
	labels := []string{AttrOperationName("").Key(), AttrSystem("").Key(), AttrTokenType("").Key(), AttrRequestModel("").Key(), server.AttrPort("").Key(), AttrResponseModel("").Key(), server.AttrAddress("").Key()}
	return ClientTokenUsage{HistogramVec: prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Name: "gen_ai_client_token_usage",
		Help: "Measures number of input and output tokens used",
	}, labels)}
}

func (m ClientTokenUsage) With(operationName AttrOperationName, system AttrSystem, tokenKind AttrTokenType, extras ...interface {
	...
	return m.HistogramVec.WithLabelValues(operationName.Value(), system.Value(), tokenKind.Value(), extra.GenAiRequestModel().Value(), extra.ServerPort().Value(), extra.GenAiResponseModel().Value(), extra.ServerAddress().Value())
}
